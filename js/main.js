...
async function init() {
    ...
    if(buttonTetrahedron) buttonTetrahedron.addEventListener('click', () => transform(targets.tetrahedron, 2000));
...
function createTetrahedronLayout() {
    ...
    const perFaceBase = Math.floor(objects.length / 4);
    const extra = objects.length % 4;
    const perFaceCounts = [0, 1, 2, 3].map((i) => perFaceBase + (i < extra ? 1 : 0));
    const faceNormals = faces.map(([a, b, c]) => {
        const v0 = vertices[a];
        const v1 = vertices[b];
        const v2 = vertices[c];
    ...
    for (let faceIndex = 0; faceIndex < 4; faceIndex++) {
        const count = perFaceCounts[faceIndex];
        for (let k = 0; k < count; k++) {
            const u = (k + 0.5) / count;
            const v = (k * 0.6180339887) % 1;
            const r = Math.sqrt(u);
            const a = 1 - r;
            const b = r * (1 - v);
            const c = r * v;
            const [i0, i1, i2] = faces[faceIndex];
            const point = new THREE.Vector3()
                .addScaledVector(vertices[i0], a)
                .addScaledVector(vertices[i1], b)
                .addScaledVector(vertices[i2], c);

            const object = new THREE.Object3D();
            object.position.copy(point);
            object.lookAt(point.clone().add(faceNormals[faceIndex]));
